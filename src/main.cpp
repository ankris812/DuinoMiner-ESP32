...
const char AUX_SETTINGS[] PROGMEM = R"(
{
  "title": "Settings",
  "uri": "/settings",
  "menu": true,
...
const char AUX_SETTINGS_SAVE[] PROGMEM = R"(
{
  "title": "Settings",
  "uri": "/settings_save",
  "menu": false,
  "element": [
    {
      ...
      "style": "display:block;margin:1.0em auto;font-size:1.17em;font-weight:bold"
    }
  ]
}
)";
...
// Settings
void handleRoot() {
  // Redirect to AUX page
  String auxPage = String("http://") + server.client().localIP().toString() + String("/_ac");
  server.sendHeader("Location", auxPage, true);
...
void getParams() {
  preferences.begin("DuinoMiner", true);
  DUCO_USER = preferences.getString("user", "");
  RIG_ID = preferences.getString("rigid", "Auto");
  MINER_KEY = preferences.getString("minerkey", "None");
  RIG_IDENTIFIER = (RIG_ID == "Auto") ? DEVICE + "-" + String((char *)chip_id) : RIG_ID;
  preferences.end();
}

String getAuxParam(AutoConnectAux& aux, String id) {
  String tmp = aux[id].value;
  tmp.trim();
  return tmp;
}

String loadAuxParams(AutoConnectAux& aux, PageArgument& args) {
  getParams();
  aux.setElementValue("user", String(DUCO_USER));
  aux.setElementValue("rigid", RIG_ID);
  aux.setElementValue("minerkey", String(MINER_KEY));
  return String("");
}

String saveAuxParams(AutoConnectAux& aux, PageArgument& args) {
  AutoConnectAux& settings = *portal.aux(portal.where());
  preferences.begin("DuinoMiner", false);
  preferences.putString("user", getAuxParam(settings, "user"));
  preferences.putString("rigid", getAuxParam(settings, "rigid"));
  preferences.putString("minerkey", getAuxParam(settings, "minerkey"));
  preferences.end();
  getParams();
  return String("");
}

String getRSSIPercentage()
{
  int dBm = WiFi.RSSI();
  if(dBm <= -100)
    return "0";
...
/*void dashboard() {
  ...
  s.replace("@@DEVICE@@", DEVICE);
  s.replace("@@ID@@", RIG_IDENTIFIER);
...
void setup() {
  ...
  snprintf((char *)chip_id, 23, "%04X%08X", (uint16_t)(chipid >> 32), (uint32_t)chipid);  // Storing the 48 bit chip chip_id into a char array.
  walletid = random(0, 2811);
  getParams();

  //Configure webserver
  IPAddress ip = IPAddress(192,168,4,1);
  config.title = "DuinoMiner";
  config.autoReconnect = true;
  config.apid = RIG_IDENTIFIER;
  config.hostName = RIG_IDENTIFIER;
  config.apip = ip;
  config.gateway = ip;
  config.netmask = IPAddress(255,255,255,0);
  config.psk = "password";
  config.menuItems = AC_MENUITEM_CONFIGNEW | AC_MENUITEM_OPENSSIDS | AC_MENUITEM_UPDATE | AC_MENUITEM_HOME | AC_MENUITEM_DELETESSID;
  config.ota = AC_OTA_BUILTIN;
  server.on("/", handleRoot);
  portal.config(config);
  portal.load(AUX_SETTINGS);
  portal.load(AUX_SETTINGS_SAVE);
  AutoConnectAux* settings = portal.aux("/settings");
  if (settings) {
    PageArgument  args;
    loadAuxParams(*settings, args);
  }
  portal.on("/settings", loadAuxParams);
  portal.on("/settings_save", saveAuxParams);
